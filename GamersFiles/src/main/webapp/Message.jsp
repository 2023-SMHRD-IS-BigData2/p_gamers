	...
	<div>
		<section id="two">
			<div class="inner">
				...
				<div id="message_box">
					<h3><%=login.getM_nick()%>님이 받은 쪽지 <span class="col1"><a
							href="sendMessage.jsp">쪽지보내기</a></span><span class="col2"><a
							href="DelMessageService.do?M_id=<%=login.getM_id()%>"
							class="button next scrolly">전체삭제하기</a></span>
					</h3>
					<!-- 쪽지 리스트가 출력되는 영역(게시판 모양) -->
					<div>
						<div id="message">
							<div class="tit">
								<span class="col1">번호</span> <span class="col2"
									id="message_title">제목</span> <span class="col3">보낸이</span> <span
									class="col4">발송시간</span> <span class="col5">비고</span>
							</div>
							<%
							for (int i = 0; i < messagesOnPage.size(); i++) {
							%>
							<div class="cuntent">
								<span class="col1"><%=startIndex + i + 1%></span> <span
									class="col2"><a
									href="MessageDetail.jsp?E_num=<%=messagesOnPage.get(i).getE_num()%>"><%=messagesOnPage.get(i).getE_title()%></a></span>
								<span class="col3"><%=messagesOnPage.get(i).getE_sender()%></span>
								<span class="col4"><%=messagesOnPage.get(i).getE_date().substring(5, 16)%></span>
								<span id="del" class="col5"><a
									href="DelMessageService.do?e_num=<%=messagesOnPage.get(i).getE_num()%>">🗑</a>
								</span>
							</div>
							<%
							}
